...
#define ALLOCATE(type, count) \
    ((type *)memory_mutator_reallocate(NULL, 0, sizeof(type) * (count)))

/// Makro that frees the memory used by a given type at the position specified by the pointer
#define FREE(type, pointer) \
    (memory_mutator_reallocate(pointer, sizeof(type), 0))

/// Makro that dealocates an existing dynamic array
#define FREE_ARRAY(type, pointer, oldCount) \
    (memory_mutator_reallocate(pointer, sizeof(type) * (oldCount), 0))
