...
void optimization_module_init(void) {
    ...
    optimization_pass_entry_t passes[] = {{.name = "dead_code_detection",
    ...
    for (size_t i = 0; i < num_passes; i++) {
        if (g_global_pipeline->pass_count >= g_global_pipeline->capacity) {
            ...
            g_global_pipeline->passes =
                realloc(g_global_pipeline->passes, sizeof(optimization_pass_entry_t) * g_global_pipeline->capacity);
        }
        g_global_pipeline->passes[g_global_pipeline->pass_count++] = passes[i];
    }

    // Sort passes by priority
    qsort(g_global_pipeline->passes, g_global_pipeline->pass_count, sizeof(optimization_pass_entry_t),
...
optimization_stats_t optimization_pipeline_run_chunk(chunk_t * chunk, const char * chunk_name) {
        ...
        for (size_t i = 0; i < g_global_pipeline->pass_count; i++) {
            ...
            if (g_global_pipeline->verbose) {
                printf("  [%s] %s: removed %zu instr, folded %zu const, eliminated %zu branches\n", chunk_name,
                       entry->name, result.instructions_removed, result.constants_folded, result.branches_eliminated);
            }
...
void optimization_print_stats(const optimization_stats_t * stats) {
    ...
    printf("  Time: %" PRIu64 " ns (%.2f us)\n", stats->time_ns, (double)stats->time_ns / 1000.0);
}
