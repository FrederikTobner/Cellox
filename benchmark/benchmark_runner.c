...
static benchmark_config_t benchmarks[] = {
    ...
    [BENCHMARK_INSTANTIATION] = {.benchmarkName = "Instantiation",
                                 .benchmarkFilePath = "Instantiation.clx",
                                 .executionCount = 3},
    [BENCHMARK_METHOD_CALL] = {.benchmarkName = "Method Call",
                               .benchmarkFilePath = "MethodCall.clx",
                               .executionCount = 3},
    [BENCHMARK_NEGATE] = {.benchmarkName = "Negate", .benchmarkFilePath = "Negate.clx", .executionCount = 3},
    [BENCHMARK_PROPERTIES] = {.benchmarkName = "Properties",
...
static void benchmark_runner_execute_benchmark(benchmark_config_t benchmark, bool custom, FILE * filePointer) {
    ...
    fprintf(filePointer, "%9gs | %9gs | %9gs | %s\n", combined_execution_duration / benchmark.executionCount,
            min_execution_duration, max_execution_duration, benchmark.benchmarkName);
    if (!custom) {
        free(filePath);
    }
    free(measured_time);
