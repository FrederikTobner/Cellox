:: Creates installer and zipped executable of the Compiler for x86 and x64 using GCC (Gnu Compiler collection)
@ECHO OFF
:: Configuring CMake and building for x86 and x64 using gcc
ECHO Configuring CMake for x64 ...
cmake -B ../build/gcc_x64 -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTS=OFF -G Ninja -DCMAKE_C_COMPILER=C:/MinGW64/bin/gcc.exe ..
ECHO Building Interpreter for x64 ...
cmake --build ./build/gcc_x64 --config Release --target Cellox
ECHO Configuring CMake for x86 ...
cmake -B ../build/gcc_x86 -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTS=OFF -G Ninja -DCMAKE_C_COMPILER=C:/MinGW32/bin/gcc.exe ..
